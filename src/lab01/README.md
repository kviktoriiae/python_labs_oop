...
- Метод **setter**: выполняет валидацию + через него можно поменять владельца
...
- Магические методы: `__str__`(строковое представление), `__repr__`(технический вывод для отладки), `__eq__`(сравнение по номеру счёта)
- Состояние объекта : открытый/закрытый (операции не могут выполняться если счет закрыт)
